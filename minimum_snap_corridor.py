...
class minimum_snap_corridor(minimum_snap):
    ...
    def getConstrainMtx(self, waypoints, n_order=7, number_sampling=4, corridor=0.7):
        ...
        for k in range(0, segment - 1):  # index number
            ...
            mtxG_middle[k + segment - 1, k * (n_order + 1):(k + 1) * (n_order + 1)] = -np.array(
                self.getCoeff(0, n_order, T[k]))
        ...
        for k in range(0, segment):  # index number
            ...
            for i, t in enumerate(time[k]):
                ...
                #####
                mtxG_sample[k * 2 * number_sampling + 2 * i, k * (n_order + 1):(k + 1) * (n_order + 1)] = self.getCoeff(
                    0, n_order, t)
                mtxG_sample[k * 2 * number_sampling + 2 * i + 1, k * (n_order + 1):(k + 1) * (n_order + 1)] = -np.array(
                    self.getCoeff(0, n_order, t))
        ...
        for n in range(1, segment):
            for k in range(0, d_order):
                mtxA_continue[(n - 1) * d_order + k, (n - 1) * (n_order + 1):(n * (n_order + 1))] = self.getCoeff(k,
                                                                                                                  n_order,
                                                                                                                  T[
                                                                                                                      n - 1])
                mtxA_continue[(n - 1) * d_order + k, n * (n_order + 1):((n + 1) * (n_order + 1))] = -np.array(
