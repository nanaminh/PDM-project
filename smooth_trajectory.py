...
def getCoeff(k, n_order, ts):
    ...
    """
    coefficient = [0] * (n_order + 1)
    if k == 0:
        for i in range(0, n_order + 1):
            coefficient[i] = ts ** i
    else:
        for i in range(k, n_order + 1):  # order number
            coefficient[i] = np.math.factorial(i) / np.math.factorial(i - k) * ts ** (i - k)
...
def getConstrainMtx(waypoints, n_order, dim):
    ...
    mtxb = np.zeros(
        ((n_order + 1) * segment, 1))  # segment -> number of the segment, n_order -> order of the polynominal
    ...
    for n in range(2, segment + 1):
        for k in range(1, 3):  # 1-6
            ...
            mtxA[2 * segment + 6 + (n - 2) * 6 + k - 1, (n - 2) * (n_order + 1):((n - 1) * (n_order + 1))] = getCoeff(k,
                                                                                                                      n_order,
                                                                                                                      1)  # error:bad operator - for list
            mtxA[2 * segment + 6 + (n - 2) * 6 + k - 1, (n - 1) * (n_order + 1):(n * (n_order + 1))] = -np.array(
                getCoeff(k, n_order, 0))
...
# >>>>>>> b3ee3d9297a0b4bc122d12429411ff98482c1770
#     return mtxA,mtxb
