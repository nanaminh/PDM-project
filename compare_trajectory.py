...
#     wp_counters = np.array([int((i * NUM_WP / 6) % NUM_WP) for i in range(num_drones)])
#     print("rrt", rrt.waypoint)
# >>>>>>> 30e675a8c944ea9c7a632d516257c0d3f65f1e24
    ##########################################################################################
    ...
    corridor = 0.5
    TARGET_POS, NUM_WP = mini_corridor.generateTargetPos(control_freq_hz, number_sampling=number_sampling,
                                                         corridor=corridor)
    step = 5
    #########################waypoint corridor###############################
    ...
    for i in range(index_continue, int(duration_sec * env.SIM_FREQ),
                   AGGR_PHY_STEPS):  ##duration_sec*env.SIM_FREQ, total time? AGGR_PHY_STEPS time step
