...
class InformedRRT:
    ...
    def __init__(self, start_pos, goal_pos, first_path=True, previous=None):
        ...
        self.goal_index = -1  # True index of goal node, for back tracing
        self.tree = []
...
if __name__ == "__main__":
    ...
    informed_rrt = InformedRRT(lowWallSeq[0], lowWallSeq[1])
    ...
    informed_rrt = InformedRRT(crawlSeq[0], crawlSeq[1], first_path=False, previous=prev)
    ...
    informed_rrt = InformedRRT(windowsSeq[0], windowsSeq[1], first_path=False, previous=prev)
    while informed_rrt.index <= 1000:
        informed_rrt.step()
        print_results(informed_rrt)
    shortest_path += informed_rrt.shortest_path
    ...
    informed_rrt = InformedRRT(tunnelSeq[0], tunnelSeq[1], first_path=False, previous=prev)
    while informed_rrt.index <= 1000:
        informed_rrt.step()
        print_results(informed_rrt)
    shortest_path += informed_rrt.shortest_path
