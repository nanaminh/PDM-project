...
# Group:10
# Reference to RO47001 Robot Dynamics & Control assignment 3 template code
#######
import numpy as np
...
def tj_from_multiple_lines(start_pos, end_pos, control_freq_hz):
    """
    input: start_pos,end_pos  waypoints,shape[num_waypoints-1,3]
    output: desired_pos   array
    """
    if np.shape(start_pos) != np.shape(end_pos):
        raise ValueError("start_pos should be SAME SHAPE with end_pos")
    ...
    TARGET_POS = np.vstack(
        [tj_from_line(start_pos[i], end_pos[i], t_ttl[i], control_freq_hz) for i in range(num_segment)])
...
# #print(start_pos[0])
# target,nm=tj_from_multiple_lines(start_pos,end_pos,24)
# print( nm)
